...
(async function() {
    ...
    window.startBulkMigration = async function() {
        console.log("🚀 Starting Year 1 Master Migration...");
        await signInAnonymously(auth);
        const col = collection(db, 'artifacts', firebaseConfig.appId, 'public', 'data', 'questions');
        for (const item of window.migrationPool) {
            await addDoc(col, { ...item, type: "unified", chapter: "Year 1 Review", grade: "Web Design 1" });
        }
        console.log("🏁 Year 1 Review Migration Complete! (165 Items Pushed)");
    };
