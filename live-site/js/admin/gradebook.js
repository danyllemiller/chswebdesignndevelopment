// js/admin/gradebook.js
/**
 * CHS Web Design - Master Teacher Gradebook Controller
 * This script powers the administrative gradebook interface, handling real-time Firebase data synchronization 
...
import { collection, getDocs, doc, setDoc, updateDoc, deleteField, writeBatch, FieldPath, onSnapshot, getDoc } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
...
function getAssignmentCategory(name, courseKey) {
    ...
    if (lowerName.startsWith('tc-') || lowerName.includes('timeclock')) {
        // CS routes timeclock to Assignment (60%), Web Design routes to Career (10%)
    ...
    return 'assignment'; // Default for Labs, Worksheets, Ch #, etc.
}

let allStudents = [];
let allGrades = {};
let allAssignments = {}; // Stores explicitly created assignments and metadata (due dates)
let calendarConfig = null; // Used to determine A/B day logic for timeclocks
let unsubscribeGrades = null;
let unsubscribeAssignments = null;
let privacyMode = false; // Global toggle for hiding student names
let showSummaryColumns = true; // Global toggle for summary columns
let currentSortMode = 'lastName'; // Global toggle for tracking current sort state
window.earliestSubmissions = {}; // Global tracker for auto-derived due dates based on first submissions

// Aggressive cleaning to ensure "Ch8-Summative-Exam" matches "Ch8 Summative Exam [20 pts]"
const cleanKey = (str) => {
    if (!str) return "";
    return str.toString()
              .replace(/\s*[\[\(]\d+\s*pts?[\]\)]/i, '') // Remove points tag
              .replace(/[^a-zA-Z0-9]/g, '')            // Remove all special chars/dashes
              .toLowerCase()
              .trim();
};
...
function injectControls() {
    ...
    if (btnSyncSheets) {
        btnSyncSheets.addEventListener('click', async (e) => {
            ...
            try {
                // Your active deployment URL for the webhook
                ...
                if (json.result === "success" && json.data) {
                    ...
                    Object.keys(json.data).forEach(studentId => {
                        ...
                        Object.keys(incomingGrades).forEach(incomingKey => {
                            ...
                            // Match keys regardless of points tags or dashes
                            ...
                            // Fill if empty, or overwrite if incoming is HIGHER
                            ...
                            if (shouldUpdate) {
                                updatesForStudent[match] = incomingData;
                                hasUpdates = true;
                            }
                        });
                        ...
                        if (hasUpdates) {
                            ...
                            batch.set(gradeRef, updatesForStudent, { merge: true });
                            opsCount++;
                        }
                    });
                    ...
                    if (opsCount > 0) {
                        await batch.commit();
                        alert(`Sync complete! Updated grades for ${opsCount} students based on highest scores.`);
...
function updatePeriodDropdown() {
    ...
    const periods = [...new Set(allStudents.map(s => s.period))].filter(p => p && p !== 'Teacher').sort();
    const prefixes = [...new Set(periods.map(p => p.split('-')[0]))].filter(Boolean).sort();
    let html = '<option value="" disabled>Select a class...</option>';
    prefixes.forEach(prefix => {
        let name = prefix === 'WD1' ? 'Web Design 1' : prefix === 'WD2' ? 'Web Design 2' : prefix === 'CS' ? 'Computer Science' : prefix;
...
async function loadData() {
    try {
        const rosterSnap = await getDocs(collection(db, 'artifacts', appId, 'public', 'data', 'roster'));
        allStudents = [];
        rosterSnap.forEach(doc => { 
            const d = doc.data();
            if (d.period !== "Teacher") {
                const realId = d.studentId || (d.id && String(d.id).length < 15 ? d.id : null) || 'Pending Setup';
                allStudents.push({ 
                    ...d, 
        ...
        const configSnap = await getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'calendarConfig', 'settings'));
        if (configSnap.exists()) calendarConfig = configSnap.data();

        onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'assignments', 'registry'), (s) => { 
            allAssignments = s.exists() ? s.data() : {}; 
            applyFiltersAndRender(); 
        });

        onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'grades'), (s) => {
            allGrades = {};
            s.forEach(doc => allGrades[doc.id] = doc.data());
            // DOM WIPE GLITCH FIX: Prevent redraw while focused
            if (!document.querySelector('.inline-edit-input')) applyFiltersAndRender();
        });
...
function isAssignmentVisible(name, period) {
    ...
    if (allAssignments[name]?.targetCourse && allAssignments[name].targetCourse !== 'All' && !period.includes(allAssignments[name].targetCourse)) return false;
    return true;
}
...
function abbreviateAssignmentName(name) {
    ...
    // Normalize architectural labels and colons into the standard dash formatting structure
    ...
    // Fallback: If no hyphens were present but spaces are, convert the first boundary space into a hyphen bound
    ...
    if (abbr.includes('-')) {
        ...
        // Remove left-over quotes, brackets, or code variables from titles
        let suffixWords = parts.slice(1).join('-').trim().replace(/['"()\[\]]/g, '').split(/[\s\/]+/); 
        
        // Smart shift: skip trivial string tokens like "the" to yield higher context items (e.g. Ch5-Bones)
...
function renderGradebook(students, grades, currentPeriod) {
    ...
    // CALC ROWS: DUE DATES, POSSIBLE, CLASS AVERAGES
    ...
    sortedKeys.forEach(key => { 
        ...
        let dateText = '-';
        if (info.dueDate) {
            const [y, m, d] = info.dueDate.split('-');
    ...
    sortStudentsArray(students).forEach((s, rowIndex) => {
        const sGrades = grades[s.studentId] || {};
        const courseKey = s.period.startsWith('WD1') ? 'WD1' : s.period.startsWith('WD2') ? 'WD2' : 'CS';
        ...
        sortedKeys.forEach(key => {
            ...
            const score = g ? (typeof g === 'object' ? g.score : g) : "";
            if (score !== undefined && score !== null && score !== "" && score !== "EX") {
    ...
    if (cell && !target.classList.contains('inline-edit-input')) {
        ...
        const save = async () => {
            ...
            if (String(final) !== String(currentScore)) {
                ...
                try {
                    const gradeRef = doc(db, 'artifacts', appId, 'public', 'data', 'grades', studentId);
                    await setDoc(gradeRef, { [assignment]: { score: final, max: Number(currentMax), timestamp: new Date().toISOString() } }, { merge: true });
                ...
                } catch (error) { console.error(error); cell.innerHTML = originalHTML; }
        ...
        input.onkeydown = (e) => {
            // FIX: Explicitly handle both keyboard and numpad Enter, and Tab/Arrows
...
async function saveAddCol() {
    ...
    await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'assignments', 'registry'), { [finalName]: { maxPoints: pts, dueDate: date, instructions: inst, targetCourse: course, periodDueDates: periodDates, created: new Date().toISOString() } }, { merge: true });
...
async function saveColEdit() {
    ...
    const batch = writeBatch(db);
    const regRef = doc(db, 'artifacts', appId, 'public', 'data', 'assignments', 'registry');
    batch.set(regRef, { [final]: { maxPoints: pts, dueDate: date, periodDueDates: periodDates, instructions: document.getElementById('editColInstructions').value, targetCourse: document.getElementById('editColCourse').value } }, { merge: true });
    if (final !== old) batch.update(regRef, new FieldPath(old), deleteField());
    await batch.commit();
...
async function confirmDeleteCol() {
    ...
    const regRef = doc(db, 'artifacts', appId, 'public', 'data', 'assignments', 'registry');
    await updateDoc(regRef, new FieldPath(target), deleteField());
