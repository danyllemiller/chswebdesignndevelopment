// js/student-payroll.js
/**
 * Student Payroll & Timesheet Viewer
 ...
 * - FIX: Removed orderBy() query requirement to prevent Firebase indexing crash.
 */
...
function getExpectedDuration(dateStr, periodStr) {
    ...
    let dayType = null;
    if (calendarConfig.dates && calendarConfig.dates.A && calendarConfig.dates.A.includes(dateStr)) dayType = "A";
    else if (calendarConfig.dates && calendarConfig.dates.B && calendarConfig.dates.B.includes(dateStr)) dayType = "B";
    else if (calendarConfig.dates && calendarConfig.dates.A_MIN && calendarConfig.dates.A_MIN.includes(dateStr)) dayType = "A_MIN";
    else if (calendarConfig.dates && calendarConfig.dates.B_MIN && calendarConfig.dates.B_MIN.includes(dateStr)) dayType = "B_MIN";
    
    if (!dayType) return 90; 
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        try {
            ...
            calendarConfig = configSnap.exists() ? configSnap.data() : { ...defaultCalendarConfig };

            const rosterRef = collection(db, 'artifacts', appId, 'public', 'data', 'roster');
            const rosterQuery = query(rosterRef, where("username", "==", email));
            const rosterSnap = await getDocs(rosterQuery);
            if (!rosterSnap.empty) {
                const data = rosterSnap.docs[0].data();
                if (data && typeof data === 'object') {
                    currentStudentName = `${data.firstName || ''} ${data.lastName || ''}`.trim() || 'Student';
                    if (data.role && PAY_SCALES[data.role]) {
                        currentRole = data.role;
                        currentRate = PAY_SCALES[data.role];
                    }
                }
            }
        } catch (e) {
            console.warn("Could not fetch roster for role.", e);
            calendarConfig = calendarConfig || { ...defaultCalendarConfig };
        }

        if (!isCSStudent) {
            buildPayrollUI(user.uid);
        } else {
...
async function buildPayrollUI(uid) {
    const container = document.getElementById('student-payroll-container');
    if (!container) return; 

    // Base UI Layout with Modals
    container.innerHTML = `
    ...
    try {
        const timesheetsRef = collection(db, 'artifacts', appId, 'users', uid, 'timesheets');
        // FIX: Removed orderBy('date', 'desc') to prevent silent index failures per Rule #2
        const snap = await getDocs(timesheetsRef);
        ...
        if (!snap.empty) {
            snap.forEach(doc => {
                try {
                    const data = doc.data();
                    if (!data || typeof data !== 'object') {
                        console.warn("Skipping malformed timesheet document:", doc.id);
                        return;
                    }
                    ...
                    if (!periodsMap[pp.id]) {
                        periodsMap[pp.id] = { 
                            info: pp, 
                            shifts: [], 
                            totals: { mins: 0, bonusCount: 0, basePay: 0, bonusPay: 0, gross: 0 } 
                        };
                    }

                // Calculate metrics for this specific shift
                let durationMins = 0;
                let shiftBonusCount = 0;
                
                if (data.statusIn === "On Time") shiftBonusCount++;
                if (data.statusOut === "On Time") shiftBonusCount++;

                if (data.clockInTime) {
                    if (data.clockOutTime) {
                        durationMins = Math.round((data.clockOutTime - data.clockInTime) / 60000);
                    } else {
                        durationMins = getExpectedDuration(data.date, data.period);
                    }
                }

                const hours = durationMins / 60;
                const basePay = hours * currentRate;
                const bonusPay = shiftBonusCount * ON_TIME_BONUS;
                const shiftGross = basePay + bonusPay;

                // Append to shift object for UI rendering
                data.calcMins = durationMins;
                data.calcGross = shiftGross;
                
                periodsMap[pp.id].shifts.push(data);
                
                // Add to Period Totals
                periodsMap[pp.id].totals.mins += durationMins;
                periodsMap[pp.id].totals.bonusCount += shiftBonusCount;
                periodsMap[pp.id].totals.basePay += basePay;
                periodsMap[pp.id].totals.bonusPay += bonusPay;
                periodsMap[pp.id].totals.gross += shiftGross;
                } catch (docErr) {
                    console.error("Error processing timesheet document:", doc.id, docErr);
                }
            });

            // Convert map to sorted array (descending by ID so index 0 is newest)
            payPeriods = Object.values(periodsMap).sort((a, b) => b.info.id - a.info.id);
            
            // Sort the individual shifts inside each period descending by date (so newest day is at top)
            payPeriods.forEach(p => {
                p.shifts.sort((a, b) => new Date(b.date) - new Date(a.date));
            });
            ...
            renderCurrentPeriod();
                ...
                document.getElementById('currentPeriodLabel').innerText = "No Data Found";
                document.getElementById('studentTimesheetBody').innerHTML = `<tr><td colspan="7" class="text-center p-4 text-muted fw-bold">No timesheets recorded yet. Remember to clock in!</td></tr>`;
                document.getElementById('btnPrevPeriod').disabled = true;
            }
    } catch (e) {
        console.error("Student Payroll Error:", e);
        document.getElementById('studentTimesheetBody').innerHTML = `<tr><td colspan="5" class="text-center text-danger p-4 fw-bold">Failed to load payroll data.</td></tr>`;
    }
}
...
function renderCurrentPeriod() {
    ...
    document.getElementById('btnNextPeriod').disabled = (currentPeriodIndex === 0);
    document.getElementById('btnPrevPeriod').disabled = (currentPeriodIndex === payPeriods.length - 1);
    document.getElementById('btnViewPaystub').disabled = false;

    // Update Totals Dashboard
    const overallHours = Math.floor(period.totals.mins / 60);
    const overallRemainingMins = period.totals.mins % 60;
    document.getElementById('totalHoursDisplay').innerText = `${overallHours}h ${overallRemainingMins}m`;
    ...
    period.shifts.forEach(data => {
        ...
        let durationText = data.calcMins > 0 ? `${Math.floor(data.calcMins/60)}h ${data.calcMins%60}m` : '-';
        if (data.calcMins > 0 && data.calcMins < 60) durationText = `${data.calcMins}m`;
        ...
        tableHtml += `
            <tr class="text-center">
                ...
                <td class="fw-bold text-success">$${data.calcGross.toFixed(2)}</td>
            </tr>
        `;
    });
...
function changePeriod(direction) {
    // Direction 1 = older (index increases), Direction -1 = newer (index decreases)
...
function showPaystubModal() {
    ...
    // Calculate YTD Gross (Sum of all periods with ID <= current period ID)
    // Since index 0 is newest, older periods have higher indexes but lower IDs.
    let ytdGross = 0;
    payPeriods.forEach(p => {
        if (p.info.id <= period.info.id) {
            ytdGross += p.totals.gross;
        }
    });

    // 🧾 TAX MATH (Realistic Withholdings)
    // Federal Tax ~10%, Social Security 6.2%, Medicare 1.45%
    ...
    const totalHoursDec = (period.totals.mins / 60).toFixed(2);

    // Populate Modal
    document.getElementById('stubPeriodDates').innerText = `${period.info.startStr} to ${period.info.endStr}`;
    document.getElementById('stubPayDate').innerText = period.info.endStr; // Assume paid on last day
    
    document.getElementById('stubRegHours').innerText = totalHoursDec;
    document.getElementById('stubRegTotal').innerText = `$${period.totals.basePay.toFixed(2)}`;
    
    document.getElementById('stubBonusQty').innerText = period.totals.bonusCount;
    document.getElementById('stubBonusTotal').innerText = `$${period.totals.bonusPay.toFixed(2)}`;
    ...
    const modal = new bootstrap.Modal(document.getElementById('paystubModal'));
    modal.show();
}