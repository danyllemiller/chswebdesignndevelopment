/** /js/examLogic.js
 * WEB DESIGN SUMMATIVE EXAM ENGINE (PROCTOR EDITION)
 * - Dedicated engine for Web Design students.
 * - Loads /student/notes.html for the side-by-side Digital Notebook.
 * - Retains standard randomized pooling logic.
 */
...
// Global Variables (Now populated via firebase-init.js import)
let db, auth, appId;
// We need these imports for database operations
let setDoc, doc, collection, query, where, getDocs, deleteDoc, getDoc, onAuthStateChanged, signOut;
...
// THE FIX: Import from your master config file
async function initCloudStorage() {
    console.log("Exam Engine: Importing master Firebase config...");
    try {
        // Dynamically import the shared config
        const fbInit = await import('/js/firebase-init.js');
        ...
        // Grab the necessary Firestore/Auth methods from the CDN
        ...
        console.log("✅ Exam Engine: Firebase master config loaded successfully.");
            ...
            if (u) {
                ...
                try {
                    ...
                    if (!snap.empty) {
                        ...
                        console.log(`✅ Exam Engine: Identity Verified [${fName} ${lName}]`);
                    } else {
                        console.warn("🔐 Security Alert: Ghost Session. Signing out.");
                        await signOut(auth);
                    ...
                    if (container) {
                        container.innerHTML = `<div class="alert alert-danger p-4 text-center">Database error: ${err.message}</div>`;
                    }
                }
            } else {
                console.log("Exam Engine: No user logged in.");
                window.location.replace(`/login.html?redirect=${encodeURIComponent(window.location.pathname)}`);
            }
        });
    } catch (e) {
        ...
        const container = document.getElementById('exam-container');
        if (container) {
            container.innerHTML = `<div class="alert alert-danger p-4 text-center">Module Load Error: ${e.message}</div>`;
        }
    }
}
initCloudStorage();
    ...
    document.addEventListener('visibilitychange', () => {
        if (examIsActive && document.visibilityState === 'hidden') {
            ...
            if (tabSwitchCount === 1) {
                showDacAlert(
                    "⚠️ SECURITY WARNING", 
                    "You have switched tabs or left the exam window. This is your ONLY warning. If you leave the exam screen again, your test will be automatically submitted."
                );
...
function initExam(config) {
    console.log("Exam Engine: initExam triggered.");
    ...
    webhookUrl = config.webhookUrl || "https://script.google.com/macros/s/AKfycbxNCMXZ5zBtBLSqalNUh6bGzSZwcp7GfUzpLb2VgzcARaNt7x1biA_9jdK64_mDfy6ajA/exec";
        ...
        <div class="card shadow border-primary mx-auto" style="max-width: 550px;">
            ...
            <div class="card-body p-4 text-center">
                ...
                <div class="bg-site-secondary p-3 rounded border border-danger mb-4 text-center">
                    ...
                    <p class="x-small text-muted mb-0">Your Digital Notebook will automatically open side-by-side with the exam. You are strictly prohibited from switching to other browser tabs.</p>
                </div>
                ...
                <div class="col-lg-7 col-xl-7 no-print">
                    <div class="card shadow-sm h-100 border-primary">
                        ...
                        <div class="card-body p-0" style="height: 80vh; min-height: 600px;">
                            <iframe src="/student/notes.html" onload="try{const doc=this.contentWindow.document; const s=doc.createElement('style'); s.innerHTML='nav, footer, .footer, .site-footer, #nav-placeholder, #footer-placeholder, #footer-nav, .back-to-top { display: none !important; opacity: 0 !important; pointer-events: none !important; height: 0 !important; }'; doc.head.appendChild(s);}catch(e){}" style="width: 100%; height: 100%; border: none;"></iframe>
                        </div>
                    </div>
                </div>
            </div>
...
async function processSubmission() {
    ...
    try {
        ...
        finalScore = serverData.score !== undefined ? Number(serverData.score) : 0; 
        finalTotal = serverData.total !== undefined ? Number(serverData.total) : examQuestions.length; 
        finalPercentage = serverData.percentage !== undefined ? Number(serverData.percentage) : 0; 
        ...
        if (db && studentId) {
            ...
            let finalAssignmentKey = "Summative-Exam";
            const unitNumMatch = chapterTitle ? chapterTitle.match(/(?:unit|chapter|ch)\s*(\d+)/i) : null;

            if (unitNumMatch && unitNumMatch[1]) {
                const isUnit = chapterTitle.toLowerCase().includes('unit');
                const prefix = isUnit ? 'Unit' : 'Ch';
                finalAssignmentKey = `${prefix}${unitNumMatch[1]}-Summative-Exam`;
            } else if (chapterTitle) {
                const cleanName = chapterTitle.replace(/[^a-zA-Z0-9]/g, '');
                finalAssignmentKey = `${cleanName}-Summative-Exam`;
            }
            ...
            if (gradeSnap.exists() && gradeSnap.data()[finalAssignmentKey] !== undefined) {
                ...
                let existingScore = typeof existingData === 'object' ? (Number(existingData.score) || 0) : (Number(existingData) || 0);
        ...
        serverFeedback.forEach(item => {
            feedbackMap[item.question.trim()] = item.hint || "Review this concept in the chapter material.";
        });
        ...
        let retakeMsg = isRetake ? `<div class="alert alert-warning fw-bold mt-3"><i class="fas fa-exclamation-triangle"></i> Score is below 80%. You need to retake this test.</div>` : "";
...
setTimeout(() => {
    if (examQuestions && examQuestions.length === 0 && typeof window.onload === 'function') {
        try { window.onload(); } catch(e) { console.error("Failsafe trigger error", e); }
    }
}, 500);