 ...
 * NOTE: Reflection/Grading features have been moved to the Chapter Pages.
 */
import { onAuthStateChanged } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-auth.js";
import { doc, setDoc, addDoc, deleteDoc, collection, query, where, getDocs, onSnapshot } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
...
async function startFileManager(user) {
    if (!user) return;
    ...
    try {
        ...
        if (!snap.empty) {
            ...
            myOwnStudentId = currentStudentData.studentId || snap.docs[0].id; 
            currentStudentId = myOwnStudentId;
            ...
            // --- TEACHER HELP REQUEST LOGIC ---
            let currentStudentDocId = snap.docs[0].id;
            const helpBtn = document.getElementById('btn-request-help');
            
            if (helpBtn) {
                helpBtn.classList.remove('d-none');
                let isHelpRequested = currentStudentData.helpRequested || false;
                ...
                helpBtn.onclick = async () => {
                    ...
                    try {
                        const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'roster', currentStudentDocId);
                        await setDoc(docRef, { helpRequested: isHelpRequested }, { merge: true });
                        updateHelpUI();
                    } catch (e) {
            ...
            document.getElementById('file-list-body').innerHTML = `<tr><td colspan="5" class="text-center p-5 text-danger fw-bold"><i class="fas fa-exclamation-triangle me-2"></i>Roster profile not found. Please contact your instructor.</td></tr>`;
        }
    } catch (e) {
...
function attachFileListeners(studentId) {
    ...
    document.querySelectorAll('.move-file-btn').forEach(btn => {
        btn.onclick = async (e) => {
            ...
            if (targetDir !== null) {
                ...
                if (newPath !== oldPath) {
                    btn.innerHTML = '⏳';
                    await moveHostGatorItem(oldPath, newPath, studentId);
                }
            }
        };
    });
    ...
    document.querySelectorAll('.share-file-btn').forEach(btn => {
        btn.onclick = (e) => {
            e.stopPropagation();
            askForShareDestination(btn.dataset.name, btn.dataset.url, false);
        };
    });
    ...
    document.querySelectorAll('.share-folder-btn').forEach(btn => {
        btn.onclick = (e) => {
            e.stopPropagation();
            askForShareDestination(btn.dataset.name, btn.dataset.url, true);
        };
    });
    ...
    document.querySelectorAll('.edit-file-btn').forEach(btn => {
        btn.onclick = (e) => {
            e.stopPropagation();
            openFileEditor(btn.dataset.path, btn.dataset.url, btn.dataset.name);
        };
    });
}
...
function attachBulkActionListeners() {
    ...
    if (shareBtn) {
        shareBtn.onclick = () => handleBulkShare();
    }
    if (moveBtn) {
        moveBtn.onclick = () => handleBulkMove();
    }
    if (deleteBtn) {
...
async function handleBulkShare() {
    ...
    try {
        const rosterRef = collection(db, 'artifacts', appId, 'public', 'data', 'roster');
        const q = query(rosterRef, where("studentId", "==", targetStudentId.trim()));
        const snap = await getDocs(q);
        ...
        if (!targetUid) {
            alert("That student has not verified their account yet.");
            return;
        }
        ...
        for (let checkbox of checked) {
            ...
            if (isFolder) {
                // Share folder and all contents
                const itemsInFolder = allFilesData.filter(f => f.path.startsWith(itemPath));
                
                // Share the folder itself
                const sharedRef = collection(db, 'artifacts', appId, 'users', targetUid, 'shared_files');
                await addDoc(sharedRef, {
                    fileName: itemName,
                    url: itemUrl,
                    isFolder: true,
                    sender: `${currentStudentData.firstName} ${currentStudentData.lastName}`,
                    timestamp: Date.now()
                });
        ...
        alert("Error sharing items. Please try again.");
    }
}
...
// ==========================================
// PEER-TO-PEER FILE SHARING INBOX
// ==========================================
...
function askForShareDestination(itemName, itemUrl, isFolder = false) {
    ...
    newBtn.addEventListener('click', async () => {
        ...
        try {
            const rosterRef = collection(db, 'artifacts', appId, 'public', 'data', 'roster');
            const q = query(rosterRef, where("studentId", "==", targetStudentId));
            const snap = await getDocs(q);

            if (snap.empty) {
                errorMsg.innerText = "Student ID not found on roster. Check the number and try again.";
                errorMsg.classList.remove('d-none');
            ...
            const targetUid = snap.docs[0].data().uid;
            if (!targetUid) {
                errorMsg.innerText = "That student has not verified their account yet.";
                errorMsg.classList.remove('d-none');
                newBtn.innerHTML = 'Send Link';
                newBtn.disabled = false;
                return;
            ...
            errorMsg.innerText = "Failed to send due to a network error.";
            errorMsg.classList.remove('d-none');
        } finally {
            newBtn.innerHTML = 'Send Link';
            newBtn.disabled = false;
...
function listenToSharedFiles() {
    const sharedRef = collection(db, 'artifacts', appId, 'users', currentUserUid, 'shared_files');
    onSnapshot(sharedRef, (snap) => {
        ...
        docs.sort((a,b) => b.timestamp - a.timestamp).forEach(d => {
            const icon = d.isFolder ? '<i class="fas fa-folder text-warning me-2"></i>' : '<i class="fas fa-link text-secondary me-2"></i>';
            const isEditable = !d.isFolder && ['.html', '.css', '.js', '.txt', '.json', '.xml', '.svg'].some(ext => d.fileName.toLowerCase().endsWith(ext));
            
            html += `
                <tr class="align-middle">
                    <td class="font-monospace fw-bold text-info">${icon}${d.fileName}</td>
                    <td class="fw-medium">${d.sender}</td>
                    <td class="small text-muted font-monospace">${new Date(d.timestamp).toLocaleDateString()}</td>
                    <td class="text-end">
                        ${isEditable ? `<button class="btn btn-sm btn-outline-dark me-1" onclick="window.openSharedFileEditor('${d.url}', '${d.fileName}')" title="Edit & Save to My Workspace"><i class="fas fa-edit"></i> Edit</button>` : ''}
                        ...
                        <button class="btn btn-sm btn-outline-danger" onclick="deleteSharedFile('${d.id}')" title="Remove from list"><i class="fas fa-times"></i></button>
                    </td>
                </tr>
            `;
        });
...
window.deleteSharedFile = async (docId) => {
    if (!confirm("Remove this shared link from your inbox?")) return;
    try {
        await deleteDoc(doc(db, 'artifacts', appId, 'users', currentUserUid, 'shared_files', docId));
    } catch (e) {
...
function askForMoveDestination(message) {
    return new Promise((resolve) => {
        ...
        Array.from(allFolders).sort().forEach(folder => {
            const opt = document.createElement('option'); opt.value = folder; opt.innerText = `📁 ${folder}`; select.appendChild(opt);
        });
        ...
        const cleanup = () => { confirmBtn.removeEventListener('click', onConfirm); modalEl.removeEventListener('hidden.bs.modal', onCancel); };
        ...
        if (e.dataTransfer.items) {
            ...
            const readEntry = async (entry, path = '') => {
                if (entry.isFile) {
                    ...
                    // Set the relative path within the dropped folder structure
                    // handleUploadBatch will prepend currentFolderPath to this
            ...
            for (let item of items) {
                if (item.kind === 'file') {
                    ...
                    if (entry) {
                        if (entry.isFile) {
                            // For simple files, use getAsFile() directly (more reliable for multiple files)
                            ...
                            // For directories, use the recursive readEntry logic
...
async function handleUploadBatch(files, studentId, isSilent = false) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        // Determine the relative path within the upload
        let relativePath = file.customPath || file.webkitRelativePath || file.name;
        
        // ALWAYS prepend currentFolderPath to place files in the open folder
    ...
    if (!isSilent && status) {
        ...
        setTimeout(() => { 
            ...
            // Reset file input values so same files can be uploaded again
            const fileInput = document.getElementById('fileInput');
...
function uploadSingleFile(file, path, studentId) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            const response = await fetch(HOSTGATOR_UPLOAD_URL, { 
                method: "POST", 
                body: formData 
            });
...
// Start Engine
onAuthStateChanged(auth, (user) => { if (user) startFileManager(user); });