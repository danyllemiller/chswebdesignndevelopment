...
(async function() {
    ...
    // MIGRATION SCRIPT
    window.startBulkMigration = async function() {
        console.log("🚀 Starting Ultimate Review Master Migration...");
        await signInAnonymously(auth);
        const col = collection(db, 'artifacts', firebaseConfig.appId, 'public', 'data', 'questions');
        for (const item of window.migrationPool) {
            await addDoc(col, { ...item, type: "unified", chapter: "Ultimate Review", grade: "All" });
        }
        console.log("🏁 Ultimate Review Migration Complete! (270 Items Pushed)");
    };
})();