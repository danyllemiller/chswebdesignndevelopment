...
| course_bucket | varchar(10) | YES | NULL | **'WD1'** / **'WD2'** / **'CS'** for due_date rows whose course_id mapped to a known course; NULL for all csv/manual rows and any unmapped due_date row. NULL = visible to every student regardless of enrollment. |
...
**source='due_date' rows** are deleted and rebuilt in full every time the teacher saves due dates via `admin/due-dates.html`. Each assignment with a due date generates one event titled `"{assignment title} Due"` (type `none`). Period-specific due dates generate additional events titled `"{title} Due – Period {id}"`. Each generated row is tagged with `course_bucket` (derived server-side in `save-due-dates.php` from the assignment's real `course_id`) so the student calendar can filter to just that student's enrolled course.

**Student-facing filtering:** `GET /api/events.php?bucket=WD1` (or `WD2`/`CS`) returns only rows where `course_bucket IS NULL OR course_bucket = ?` — i.e. general school-wide dates plus that student's own due dates. Omitting `?bucket` (teacher/admin view) returns every row unfiltered. `js/calendar.js` derives the bucket client-side from the logged-in user's `section_id` prefix (`WD1-*` / `WD2-*` / `CS-*`); anything it can't classify — including teachers/admins — gets the unfiltered view, so a classification gap can never hide a real assignment.

**Used by:** `api/events.php` (GET all/filtered, POST insert, PUT update, DELETE by id), `api/save-csv.php` (DELETE csv + bulk INSERT csv), `api/admin/save-due-dates.php` (DELETE due_date + bulk INSERT due_date, tags course_bucket), `js/calendar.js` initCalendar() / getStudentCalendarBucket() / saveSingleEvent() / deleteEvent()
