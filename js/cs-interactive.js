...
// Workspace is ALWAYS accessible - don't hide it
            ...
            if (activeTab.type === 'PRE_SCALE') {
                ...
                // Check if bypasses are enabled (global config OR bypassAll flag)
                ...
                // Exam unlocked if bypass enabled OR all requirements met
                ...
                if (!examUnlocked && dom.examOverlay) {
                    ...
                    `;
                    // Wait a tick for DOM injection, then bind
                    setTimeout(() => {
...
if (chapBtn) {
                ...
                if (target === 'PRE_SCALE') {
                    ...
                    // Check requirements - but allow bypass via GLOBAL_BYPASS_CONFIG
                    const bypassWorksheets = !GLOBAL_BYPASS_CONFIG.requireWorksheets || GLOBAL_BYPASS_CONFIG.bypassAll;
                    const bypassDiagnostic = !GLOBAL_BYPASS_CONFIG.requireDiagnostic || GLOBAL_BYPASS_CONFIG.bypassAll;
                    const bypassPreScale = !GLOBAL_BYPASS_CONFIG.requirePreScale || GLOBAL_BYPASS_CONFIG.bypassAll;
                    
                    const hasPreScale = selfAssessmentsLoaded && selfAssessments[`unit${activeUnit.unitNum}`] !== undefined && selfAssessments[`unit${activeUnit.unitNum}`] > 0;
                    const hasPreTest = gradesLoaded && Object.keys(grades).some(k => k.match(new RegExp(`Unit\\s*-?\\s*${activeUnit.unitNum}`, 'i')) && k.match(/(Diagnostic|Pre-Assessment|Pre)/i));
                    ...
                    // Check if passes (bypass OR actual completion)
                    const examUnlocked = bypassPreScale || hasPreScale;
                    const diagUnlocked = bypassDiagnostic || (bypassPreScale || hasPreScale);
                    const worksUnlocked = bypassWorksheets || allChaptersDone;
                    
                    if (examUnlocked && diagUnlocked && worksUnlocked) {
                        // Requirements met OR bypassed - launch the exam
