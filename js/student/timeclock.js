...
async function resolveTodaysBellWindow() {
    ...
    try {
        const [csvText, eventsData, bellData] = await Promise.all([
            ...
            fetch('/api/events').then(r => r.ok ? r.json() : { events: [] }).catch(() => ({ events: [] })),
            fetch('/api/bell-schedule').then(r => r.ok ? r.json() : { schedule: [] }).catch(() => ({ schedule: [] })),
        ]);
...
async function checkStatusInner() {
    ...
    try {
        ...
        if (mode === 'in') {
            ...
            // Clock-in is always a real multiple-choice question -- WD1/WD2/AS
            // pull from the Workplace Readiness MC bank, CS from its chapter
            // test bank -- never a manually-typed question.
